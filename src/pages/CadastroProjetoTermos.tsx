...
// Tela: Cadastro de Projeto — Variação B (Termos e Condições)
...
interface FormData {
  ...
  termos: {
    ...
    autorizouCompartilhamento: boolean;
    aceitouNotificacoes: boolean;
  };
}
...
function CheckboxItem({ id, checked, onChange, label, badge, hasError }: CheckboxItemProps) {
  return (
    <label
      htmlFor={id}
      className="flex items-start gap-3 py-3.5 border-b border-[#1c2d44] last:border-none
    ...
    >
      ...
      {/* Custom checkbox */}
      <span
        className={`flex-shrink-0 mt-0.5 w-[18px] h-[18px] rounded-[5px] border-2
                    flex items-center justify-center transition-all duration-200
                    ${checked
                      ? "bg-blue-600 border-blue-600 ring-2 ring-blue-500/25"
                      : hasError
                        ? "bg-[#0d1626] border-red-500 ring-2 ring-red-500/20"
                        : "bg-[#0d1626] border-[#1e3352] group-hover:border-[#3b5a8a]"
                    }`}
      >
        {checked && (
      ...
      <span className={`text-sm leading-relaxed transition-colors duration-200
        ...
        {label}
        {badge && (
          <span className="ml-2 inline-block px-2 py-0.5 rounded-full text-[0.65rem]
                           font-semibold uppercase tracking-wider align-middle
...
/* ── Main Component ─────────────────────────────────── */
export default function CadastroProjetoTermos() {
  const [formData, setFormData] = useState<FormData>({
    ...
    termos: {
      ...
      autorizouCompartilhamento: false,
      aceitouNotificacoes: false,
    },
  });
  ...
  function validate(): boolean {
    ...
    if (!formData.termos.aceitouTermosUso)
      newErrors.termosUso = "Você precisa aceitar os termos de uso para continuar.";
  ...
  const setTermos = (key: keyof FormData["termos"], val: boolean) => {
    setFormData((p) => ({ ...p, termos: { ...p.termos, [key]: val } }));
    if (key === "aceitouTermosUso" && val)
      setErrors((p) => ({ ...p, termosUso: undefined }));
  };
      ...
      <div className="max-w-[760px] mx-auto pb-20">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <section className="bg-[#111827] border border-[#1c2d44] rounded-2xl p-6 mb-5
            ...
            <p className="text-xs text-slate-400 mb-5">Leia e aceite os termos para prosseguir</p>

            <div className="flex flex-col">
              <CheckboxItem
                id="term1"
                checked={formData.termos.aceitouTermosUso}
                onChange={(v) => setTermos("aceitouTermosUso", v)}
                label="Concordo com os termos de uso e política de privacidade da plataforma"
                badge="Obrigatório"
                hasError={!!errors.termosUso}
              />
              {errors.termosUso && (
                <p className="text-xs text-red-400 mb-2 pl-[30px]">{errors.termosUso}</p>
              )}

              <CheckboxItem
                id="term2"
                checked={formData.termos.autorizouCompartilhamento}
                onChange={(v) => setTermos("autorizouCompartilhamento", v)}
                label="Autorizo o compartilhamento de dados do projeto com a equipe designada"
              />

              <CheckboxItem
                id="term3"
                checked={formData.termos.aceitouNotificacoes}
                onChange={(v) => setTermos("aceitouNotificacoes", v)}
                label="Aceito receber notificações e atualizações sobre o status do projeto por e-mail"
              />
            </div>
          </section>
